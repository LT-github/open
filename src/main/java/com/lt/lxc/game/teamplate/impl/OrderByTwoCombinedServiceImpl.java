 ...
 */
@Service("TwoCombined")
public class OrderByTwoCombinedServiceImpl extends IOrderService{
	...
	public OpenResultVo countLotteryResult(OrderDTO order, List<Integer> lotteryCode) {
		...
		//首先判断总和是否为合值,如果是的直接返回不用做后续判断
		if(isAnd(count)) {
			lotteryResult.add(GameEnum.GameContent.AND.getMessage()+ dragonAndTigerResult(lotteryCode));
		}else {
			//大小和单双的组合判断
			lotteryResult.add(bigAndSmall(count,MUTIL_MEDIANT)+singleAndDouble(count));
			//大小和龙虎的组合判断
			lotteryResult.add(bigAndSmall(count,MUTIL_MEDIANT)+dragonAndTigerResult(lotteryCode));
			//单双和龙虎的组合判断
			lotteryResult.add(singleAndDouble(count)+dragonAndTigerResult(lotteryCode));
		}
	...
	public static void main(String[] args) {
		OrderByTwoCombinedServiceImpl s=new OrderByTwoCombinedServiceImpl();
		OrderDTO order=new OrderDTO();

		order.setBetsContent("大虎100");//
		order.setOdds(2.0);
